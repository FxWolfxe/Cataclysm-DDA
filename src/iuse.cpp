...
std::optional<int> iuse::pickaxe( Character *p, item *it, bool, const tripoint &pos )
{
    ...
    const bool mineable_furn = here.has_flag_furn(ter_furn_flag::TFLAG_MINEABLE, pos);
    const bool mineable_ter = here.has_flag_ter(ter_furn_flag::TFLAG_MINEABLE, pos);
    const int max_mining_ability = 70;
    ...
    // You can mine either furniture or terrain, and furniture goes first,
    // according to @ref map::bash_ter_furn()
    std::string mining_what = mineable_furn ? here.furnname(pos) : here.tername(pos);
    ...
    p->activity.targets.emplace_back( *p, it );
    p->activity.placement = here.getglobal( pos );
